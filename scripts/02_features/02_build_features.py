...
def extract_phenometrics_gs(dos_arr, smoothed, vi_name):
    ...
    WIN_SOS_SCAN = (200, None)  # DOS 201..POS for SOS detection (third-deriv max)
...
def main():
    ...
    # ── Load sowing-date lookup (Option 3) ──
    SOW_PATH = EXT / 'sowing_lookup.parquet'
    if SOW_PATH.exists():
        sowdf = pd.read_parquet(SOW_PATH)
        sowdf['field_id'] = sowdf['field_id'].astype(str)
        sowing_lookup = dict(
            zip(zip(sowdf['field_id'], sowdf['harvest_year']),
                sowdf['sowing_doy_used']))
        n_obs = (sowdf['source'] == 'observed').sum()
        print(f'Sowing lookup: {len(sowing_lookup):,} entries '
              f'({n_obs:,} observed, {len(sowing_lookup)-n_obs:,} state-median)')
    else:
        sowing_lookup = {}
        print(f'WARNING: no sowing lookup at {SOW_PATH}; falling back to DOY 258')
    ...
    for i, fy in enumerate(valid.itertuples(index=False)):
        ...
        # ── WES simulation (per-field sowing date from lookup; falls back to 258) ──
        if len(wx_fy):
            sowing_doy = int(sowing_lookup.get((fid, hy), 258))
            row['sowing_doy_used_actual'] = sowing_doy   # diagnostic
        ...
        row['sowing_doy_used']   = int(sowing_lookup.get((fid, hy), 258))
