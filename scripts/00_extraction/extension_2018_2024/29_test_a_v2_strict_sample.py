"""TEST A — V2 features filtered to V1's sample (1,830 high-quality
field-years that have flag_leaf labels), then run Phase E across all
5 candidate models. Tests whether V2's R² drop is due to sample
composition rather than methodology.

Outputs (persistent in /depot):
    extension_2018_2024/v2_strict_sample/multi_stage_models.parquet
    extension_2018_2024/v2_strict_sample/multi_stage_best.csv
    extension_2018_2024/v2_strict_sample/_run.log
"""
import sys
import time
...
def loyo(df, feat_cols, target, factory, model_name):
    ...
    for yr in sorted(df2['year'].unique()):
        ...
        if is_linear:
            ...
            steps = [('imp', SimpleImputer(strategy='median')), ('sc', StandardScaler()), ('m', factory())]
            pipe = Pipeline(steps)
...
def main():
    ...
    # FILTER: keep only V2 rows that are in V1 sample
    v1_pairs = set(zip(v1['field_id'], v1['year']))
    v2_filt = v2[v2.apply(lambda r: (r['field_id'], r['year']) in v1_pairs, axis=1)].copy()
    log(f'V2 filtered to V1 sample: {len(v2_filt):,} rows (was {len(v2):,})', fh)
    ...
    if len(df):
        ...
        log(f'\n=== BEST per stage (V2 features, V1 sample filter) ===', fh)
