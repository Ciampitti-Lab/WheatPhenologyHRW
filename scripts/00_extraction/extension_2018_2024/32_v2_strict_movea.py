"""Move A — V2 strict sample + physiological target bounds per stage.

V2 features (growing-season DOS smoothing) + V1's high-quality sample
(1,830 field-years with flag_leaf labels) + STRICT physiological DOS
bounds on each stage's target (drops out-of-range labels like
maturity_dos_obs=7 which actually refers to next year's wheat).

Outputs in /depot (persistent):
    v2_strict_movea/multi_stage_models.parquet
    v2_strict_movea/multi_stage_best.csv
    v2_strict_movea/_run.log
"""
import sys
import time
...
V2_FEAT = EXT / 'features_gs_train_2014_2017.parquet'
...
# Move A — physiological DOS bounds per stage. Anything outside is dropped.
# DOS 1 = July 1 of harvest_year - 1.
STAGE_DOS_BOUNDS = {
    'emergence':  (60,  180),   # Sep early   → Jan late
    'tillering':  (100, 260),   # Oct early   → Mar mid
    'jointing':   (200, 300),   # Jan mid     → Apr late
    'flag_leaf':  (270, 335),   # Mar late    → Jun early
    'boot':       (280, 345),   # Apr early   → Jun mid
    'heading':    (290, 350),   # Apr mid     → Jun mid
    'anthesis':   (295, 360),   # Apr late    → Jun late
    'maturity':   (320, 365),   # May late    → Jun late
}
...
def loyo(df, feat_cols, target, factory, model_name):
    ...
    # Move A — strict physiological bounds
    stage = target.replace('_dos_obs', '')
    if stage in STAGE_DOS_BOUNDS:
        lo, hi = STAGE_DOS_BOUNDS[stage]
        df2 = df2[(df2[target] >= lo) & (df2[target] <= hi)]
...
def main():
    ...
    # V2 features filtered to V1 sample (high-quality field-years)
    v1_pairs = set(zip(v1['field_id'], v1['year']))
    v2_filt = v2[v2.apply(lambda r: (r['field_id'], r['year']) in v1_pairs, axis=1)].copy()
    log(f'V2 strict sample: {len(v2_filt):,} field-years', fh)

    targets = build_targets(pheno)
    v2_filt = v2_filt.merge(targets, on=['field_id','year'], how='left')

    # Report what Move A drops per stage
    log('\n=== Move A — target bounds dropping ===', fh)
    for stage, (lo, hi) in STAGE_DOS_BOUNDS.items():
        col = f'{stage}_dos_obs'
        if col not in v2_filt.columns: continue
        n_total = v2_filt[col].notna().sum()
        within = ((v2_filt[col] >= lo) & (v2_filt[col] <= hi)).sum()
        log(f'  {stage:<10}  bounds=[{lo:>3},{hi:>3}]   '
            f'kept {within:>5} / {n_total:>5}   dropped {n_total-within:>4} '
            f'({(n_total-within)/n_total*100:.1f}%)', fh)
    ...
    for stage in STAGE_MAP:
        ...
        if target not in v2_filt.columns: continue
        n = v2_filt[target].notna().sum()
        if n < 30:
        ...
        for sname, inc_wes in strategies:
            ...
            for mname, fac in factories.items():
                ...
                t1 = time.time()
                try:
                    p, t = loyo(v2_filt, cols, target, fac, mname)
    ...
    if len(df):
        ...
        log(f'\n=== BEST per stage (V2 strict + Move A) ===', fh)
